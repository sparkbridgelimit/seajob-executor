...
async function sendHello(node, marketPage) {
  ...
  await sleep(4000); // todo 沟通列表偶尔会缺少待打开的岗位，仅 window 出现。
  ...
  await detailPage
    ...
    .catch((e) => e); // 跳转列表按钮
  await sleep(4000); // 等待消息发送
  targetNum--;

  const shortUrl = detailPageUrl.split('?').shift();
  // 已投递的公司
  myLog(`OK | ${job_task_id} | ${fullName} | [${oriSalaryMin}-${oriSalaryMax}K] | ${shortUrl}`);
